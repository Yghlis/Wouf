...
// Vérifier si la variable existe dans le cache
if (storedRole !== null) {
    // La variable existe dans le cache, vous pouvez l'utiliser
    console.log('Role récupéré du cache:', storedRole);
} else {
    // La variable n'existe pas dans le cache
    console.log('La variable de rôle n\'est pas présente dans le cache.');
}
        }).catch(err => {
