...
WA.onInit().then(async () => {
    ...
    // Supposons que vous avez configuré des zones dans Tiled
WA.room.onEnterZone('adminZone').subscribe(() => {
    // Utilisez ici le chemin vers le sprite pour le rôle 'admin'
    WA.player.setCustomPlayerSprite('path_to_admin_sprite.png');
});

WA.room.onEnterZone('userZone').subscribe(() => {
    // Utilisez ici le chemin vers le sprite pour le rôle 'user'
    WA.player.setCustomPlayerSprite('path_to_user_sprite.png');
});
