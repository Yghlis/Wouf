    ...
    function openArray() {
        WA.ui.website.open({
            ...
            window.addEventListener('message', function(event) {
                ...
                if (event.origin !== "window.addEventListener('message', function(event) {
                    // Assurez-vous que le message vient de la source attendue ou vérifiez le contenu du message
                    if (event.origin !== "https://expected-origin.com/" && event.data.type === "calendarUpdate") {
                        console.log("Data from calendar:", event.data);
                    }
                });" && event.data.type === "calendarUpdate") {
                    console.log("Data from calendar:", event.data);
                }
            });
