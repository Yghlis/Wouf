...
WA.onInit().then(() => {
    ...
    function openArray() {
        WA.ui.website.open({
         ...
         window.addEventListener('roleSelected', () => {
    // Récupérer le rôle depuis le cache local
    let storedRole = localStorage.getItem('role');
    
    // Vérifier si le rôle a été sélectionné
    if (storedRole !== null) {
        // Le rôle a été sélectionné, vous pouvez l'utiliser
        console.log('Rôle sélectionné:', storedRole);
        
        // Vous pouvez également appeler une fonction ou exécuter d'autres actions en fonction du rôle sélectionné
    } else {
        // Le rôle n'a pas été sélectionné, gérer cette situation en conséquence
