        ...
        if (!WA.player.tags.includes('admin')) {
            ...
            try {
                ...
                for (const player of players) {
                    console.log(`Player ${player.name} with tags ${player.tags.join(', ') || 'No Tags'} is near you`);
                }
            } catch (error) {
                console.error('Error while configuring player tracking or listing players:', error);
            }
        }
    });
