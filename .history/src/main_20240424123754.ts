...
WA.onInit().then(() => {
    ...
    WA.room.area.onEnter('jitsiMeetingRoom').subscribe(async () => {
        ...
        // Save and make the player's tags public
        await WA.player.state.saveVariable("tags", WA.player.tags.join(', '), {
            public: true,
            persist: true,
            ttl: 86400,  // tags will persist for one day
            scope: "world"
        });
        ...
        if (!WA.player.tags.includes('admin')) {
            ...
            for (const player of players) {
                ...
                console.log(`Player ${player.tag} is near you`);
            }
        }
    });
    ...
    WA.room.area.onEnter('clock').subscribe(() => {
        ...
        // Open a modal towards the WorkAdventure website
