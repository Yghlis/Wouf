...
WA.onInit().then(() => {
    ...
    WA.room.area.onEnter('jitsiMeetingRoom').subscribe(async() => {
        ...
        if (!WA.player.tags.includes('admin')) {
            ...
            try {
                await WA.players.configureTracking();  // Assurez-vous que le suivi des joueurs est activé.
                const players = WA.players.list();  // Obtenez la liste des joueurs autour.
                for (const player of players) {
                    // Affiche le nom du joueur et ses tags s'ils existent.
